...
export default function PostItem({ post }: { post: Post }) {
  ...
  useEffect(() => {
    if (currentIndex === 0) {
      console.log("در حال نمایش اولین تصویر هستیم");
    } else {
      console.log("تصویر شماره", currentIndex);
    }
  }, [currentIndex]);
  console.log("Current Image URL:", post.posts[currentIndex]);
  useEffect(() => {
    console.log(
      `%c[Modal Debug] showModal=${showModal}  idx=${currentIndex}`,
  ...
  return (
    <div className="w-full max-w-[470px] mx-auto pb-6 border-b border-gray-200">
      ...
      <div className="relative w-full aspect-square bg-black">
        ...
        {hasMultiple && (
          <>
            {currentIndex !== post.posts.length - 1 && (
              <button
                onClick={nextImage}
      ...
      {showModal && <ComponentModal activePost={activePost} />}
    </div>
  );
}